...
export default {
  config: {
    ...
    puppetParserArgs: {
      ...
      type: 'array',
      description: 'Arguments and/or flags to pass to parser.',
      default: [],
      items: {
        type: 'string'
      }
    ...
    return {
      ...
      lint: (activeEditor) => {
        ...
        if (atom.config.get('linter-puppet-parsing.puppetParserArgs')[0] !== '')
          args.push(...atom.config.get('linter-puppet-parsing.puppetParserArgs'));

        // add arg to parser for sub-subcommand to validate and file to validate
        args.push(...['validate', file]);
          ...
          output.split(/\r?\n/).forEach(function (line) {
            ...
            // parse the errors and warnings with end of file from puppet parser validate
              ...
              toReturn.push({
                ...
                location: {
                  file: file,
                  position: [[activeEditor.getLastScreenRow() - 1, 0], [activeEditor.getLastScreenRow() - 1, 1]],
                },
              });
            }
          });
