      ...
      lint: (activeEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-puppet-parsing.puppetExecutablePath'), args, {stream: 'stderr', allowEmptyStderr: true}).then(output => {
          ...
          output.split(/\r?\n/).forEach(function (line) {
            ...
            // same kind of parsing, but for puppet>=5.3 output
              ...
              toReturn.push({
                ...
                location: {
                  file: file,
                  position: matches_new[4] == undefined ? [[Number.parseInt(matches_new[3]) - 1, 0], [Number.parseInt(matches_new[3]) - 1, 1]] : [[Number.parseInt(matches_new[3]) - 1, Number.parseInt(matches_new[4]) - 1], [Number.parseInt(matches_new[3]) - 1, Number.parseInt(matches_new[4])]],
                },
              });
            }
