    ...
    return {
      ...
      lint: async (textEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-puppet-parsing.puppetExecutablePath'), ['parser', 'validate', '--render-as', 'json', editorFile], {stream: 'both'}).then(output => {
          ...
          // if invalid json, the file is clean or the output is a raw string; either way, continue onwards to handle each appropriately
          catch (SyntaxError) {
            // sometimes issues fail to output as json
            output.stderr.split(/\r?\n/).forEach((line) => {
              const matches = /(Error|Warning).*:\s(.*)\sat.*\.pp:(\d+):?(\d+)?/.exec(line);
              const matches_new = /(Error|Warning).*:\s(.*)\s\(file.*line: (\d+), column: (\d+)/.exec(line);
            ...
            // return the parsed info from stderr
            return toReturn;
          }

          // valid json, so iterate through array destructured object entries representing issue(s)
          ...
          // return the parsed info from stdout
          return toReturn;
        });
      }
    };
